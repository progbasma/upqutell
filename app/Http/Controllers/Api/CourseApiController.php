...
class CourseApiController extends Controller
{
    ...
    {
        $courses = Course::where('type', '1')->with('user')->where('status', 1)->latest()->get();
    ...
    public function getAllClasses()
    {
        ...
        $courses = Course::where('type', '3')->with('user', 'class')->where('status', 1)->latest()->get();
        $response = [
            'success' => true,
            'data' => $courses,
            'total' => count($courses),
    ...
    {
        $courses = Course::where('type', '2')->with('user', 'quiz')->where('status', 1)->latest()->get();
        $response = [
            'success' => true,
            'data' => $courses,
            'total' => count($courses),
    ...
    public function getQuizDetails($id)
    {
        ...
        if (isModuleActive('AdvanceQuiz') && $user) {
            $last_given_quiz = QuizTest::where('user_id', $user->id)->where('course_id', $course->id)->where('quiz_id', $course->quiz_id)->with('details')->latest()->first();
